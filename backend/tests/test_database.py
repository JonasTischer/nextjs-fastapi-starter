...
async def mock_session(mocker):
    ...
    mock_session_maker = mocker.patch("src.core.database.async_session_maker")
...
async def test_create_db_and_tables(mock_engine, mocker):
    ...
    mocker.patch("src.core.database.engine", mock_engine)
...
def test_engine_creation(mocker):
    # Mock settings
    mock_settings = mocker.patch("src.core.database.settings")
